...
test('simulateDanger with ftsSerials publishes FTS reset to each', async () => {
  ...
  const result = await business.simulateDanger([], { ftsSerials: ['5iO4', 'xkI4'] });
  ...
  assert.equal(publishLog[2]?.topic, 'fts/v1/ff/xkI4/instantAction');

  assert.equal(result.sentTopics.length, 3);
  assert.equal(result.sentTopics[1]?.topic, 'fts/v1/ff/5iO4/instantAction');
  assert.equal(result.sentTopics[2]?.topic, 'fts/v1/ff/xkI4/instantAction');
