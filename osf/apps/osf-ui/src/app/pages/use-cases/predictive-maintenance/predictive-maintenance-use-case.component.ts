...
/** Connection element IDs – not-highlighted connections get dimmed */
const UC05_CONNECTION_IDS = [
...
export class PredictiveMaintenanceUseCaseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private async loadSteps(): Promise<void> {
    try {
      const stepsUrl = 'assets/use-cases/uc-05/uc-05-predictive-maintenance.steps.json';
      const stepsData = await this.http.get<Uc05Step[]>(stepsUrl).toPromise();
      if (stepsData) {
        this.steps = stepsData;
        this.cdr.markForCheck();
      }
    } catch (error) {
      console.error('Failed to load UC-05 steps:', error);
    }
  }
  ...
  private async loadSvg(): Promise<void> {
    try {
      ...
      console.error('Failed to generate UC-05 SVG:', error);
      this.isLoading = false;
      this.cdr.markForCheck();
    }
  }
  ...
  private applyStep(index: number): void {
    ...
    const runApply = (): void => {
      const svgElement =
        ...
        this.elementRef.nativeElement?.querySelector('.predictive-maintenance-use-case__svg-wrapper svg');
      if (!svgElement) return;

      const titleEl = svgElement.querySelector('#uc05_title');
      const subtitle = svgElement.querySelector('#uc05_subtitle');
      const stepDesc = svgElement.querySelector('#uc05_step_description');

      if (index === 0) {
        svgElement.querySelectorAll('[id^="uc05_"]').forEach((el) => {
        ...
        if (this.showDescription) {
          ...
          if (stepDesc) {
            ...
            const descTitle = svgElement.querySelector('#uc05_step_description_title');
            const descText = svgElement.querySelector('#uc05_step_description_text');
        ...
        // Alle nicht-aktiven Parts werden blaß – aber nie Vorfahren von hervorgehobenen Elementen,
        // da sonst Opacity multipliziert und Highlight-Elemente ebenfalls abgedunkelt würden.
        ...
        svgElement.querySelectorAll('[id^="uc05_"]').forEach((el) => {
          ...
          el.classList.add(UC05_CONNECTION_IDS.includes(id) ? 'dim-conn' : 'dim');
        });
      }
    };
  ...
  protected getStepTitle(step: Uc05Step | undefined): string {
    if (!step) return '';
