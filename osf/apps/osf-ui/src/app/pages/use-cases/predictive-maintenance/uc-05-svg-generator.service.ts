...
export class Uc05SvgGeneratorService {
  ...
  private defs(alarmStroke: string): string {
    ...
    return `<defs>
      ...
      <style>
        ...
        .uc05-step-title { font: 700 18px "Segoe UI",Arial,sans-serif; fill: ${nightBlue}; }
  ...
  private processStepBox(step: Uc05ProcessStep, t: (k: string) => string, D: typeof ORBIS_COLORS.diagram): string {
    ...
    out += `<text x="${step.x + step.width / 2}" y="${titleY}" text-anchor="middle" class="uc05-step-title">${this.esc(t(step.titleKey))}</text>`;
    bullets.slice(0, 3).forEach((b, i) => {
      out += `<text x="${step.x + step.width / 2}" y="${titleY + 20 + i * lineH}" text-anchor="middle" class="uc05-step-bullet">${this.esc(b.replace(/^[•\-\*]\s*/, ''))}</text>`;
    });
    out += '</g>';
    return out;
  }
