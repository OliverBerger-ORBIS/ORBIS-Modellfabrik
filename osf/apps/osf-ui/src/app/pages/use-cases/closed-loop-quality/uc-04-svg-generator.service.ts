...
export class Uc04SvgGeneratorService {
  generateSvg(i18nTexts: Record<string, string>): string {
    ...
    svg += `<g id="uc04_outcome"><text x="${s.outcome.x}" y="${s.outcome.y}" text-anchor="middle" font-family="Segoe UI" font-weight="600" font-size="16" fill="${highlightGreenStrong}">${this.esc(t(s.outcome.key))}</text></g>`;

    // Footer (Disclaimer)
    svg += `<g id="uc04_footer"><text x="${s.footer.x}" y="${s.footer.y}" text-anchor="middle" class="uc04-footer">${this.esc(t(s.footer.key))}</text></g>`;
  ...
  private defs(qualityStroke: string): string {
    ...
    return `<defs>
      ...
      <style>
        ...
        .uc04-step-title { font: 700 22px "Segoe UI",Arial,sans-serif; fill: ${nightBlue}; }
        ...
        .uc04-footer { font: 400 16px "Segoe UI",Arial,sans-serif; fill: ${ORBIS_COLORS.neutralDarkGrey}; }
      </style>
    </defs>`;
  }
  ...
  private processStepBox(step: Uc04ProcessStep, t: (k: string) => string, D: typeof ORBIS_COLORS.diagram): string {
    ...
    out += `<text x="${innerCenterX}" y="${titleY}" text-anchor="middle" class="uc04-step-title">${this.esc(t(step.titleKey))}</text>`;
    bullets.slice(0, 3).forEach((b, i) => {
      out += `<text x="${innerLeft + bulletPad}" y="${bulletStartY + i * lineH}" text-anchor="start" class="uc04-step-bullet">${this.esc(b.replace(/^[•\-\*]\s*/, ''))}</text>`;
    });
    out += '</g>';
    return out;
  }
