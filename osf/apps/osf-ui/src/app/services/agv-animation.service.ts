...
import { AgvRouteService, type RouteSegment } from './agv-route.service';
...
export class AgvAnimationService {
  ...
  constructor(private readonly ftsRouteService: AgvRouteService) {}
  ...
  private startSegmentAnimation(from: string, to: string): void {
    ...
    if (!toPos) {
      console.warn('[AgvAnimationService] Cannot start animation - missing target position:', { from, to, fromCanonical, toCanonical, toPos });
      this.stopAnimation();
      return;
    }
  ...
  private updateActiveRouteSegmentsForFullPath(): void {
    ...
    for (let i = 0; i < this.animationPath.length - 1; i++) {
      ...
      if (road) {
        ...
        console.warn('[AgvAnimationService] Road not found between nodes:', { from, to, fromCanonical, toCanonical, animationPath: this.animationPath });
      }
    }
    ...
    console.log('[AgvAnimationService] Route segments set:', {
