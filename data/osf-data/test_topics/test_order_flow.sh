...
echo ""
echo "=== 1. Layout mit HBW (SVR3QA0022) setzen ==="
mosquitto_pub -h $HOST -p $PORT -t "ccu/set/layout" -f "$PROJECT_ROOT/data/osf-data/test_topics/layout_hbw_demo.json" -q 2
echo "Layout gesendet."
sleep 2

echo ""
echo "=== 2. Preloads für HBW SVR3QA0022 senden (connection, state, factsheet) ==="
PRELOADS="$SCRIPT_DIR/preloads"
# Reihenfolge: connection → state → factsheet (State zuerst = pending-Stock-Fix greift)
for f in "$PRELOADS/module_v1_ff_SVR3QA0022_connection.json" "$PRELOADS/module_v1_ff_SVR3QA0022_state.json" "$PRELOADS/module_v1_ff_SVR3QA0022_factsheet.json"; do
  sleep 0.5
  if [ -f "$f" ]; then
    TOPIC=$(jq -r '.topic' "$f")
