...
if order_id:
    if order_id.startswith('py_'):
        ...
        status = {'status': st.session_state.get('py_sequence_status', 'active'), 'context': {'orderId': 'PY-ORDER', 'module': module}}
    else:
    ...
    show_details = st.checkbox("Step Context & Payload anzeigen", value=st.session_state.get('show_step_details', False))
    ...
    for i, step in enumerate(steps):
        ...
        st.markdown(f"<div style='margin-bottom:8px;{style}'>{num_icon} {icon} {step['name']}</div>", unsafe_allow_html=True)
