...
class APSPersistentLogger:
    ...
    def __init__(self, host="192.168.0.100", port=1883, username="default", password="default", session_label=None):
        self.host = host
        self.port = port
    ...
    def _detect_process_type(self, topic, payload):
        ...
        try:
            ...
            # Order processing
            if any(keyword in topic_lower or keyword in payload_lower 
                   for keyword in ['order', 'bestellung', 'blue', 'blau', 'red', 'rot', 'yellow', 'gelb']):
                return 'order_processing'
            
            # Wareneingang
            if any(keyword in topic_lower or keyword in payload_lower 
                   for keyword in ['wareneingang', 'input', 'classify', 'klassifizierung', 'nfc']):
                return 'wareneingang'
