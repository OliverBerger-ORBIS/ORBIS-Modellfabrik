    ...
    with col1:
        ...
        if not df.empty and single_session_mode:
            ...
            if "timestamp" in df.columns:
                ...
                if (
                    ...
                    and min_time.year > 2000
                    and max_time.year > 2000
                ):  # Basic sanity check
                    # Check if time range is too small (less than 1 second)
                    time_diff = max_time - min_time
                    if time_diff.total_seconds() < 1:
                        st.info(
                            f"ℹ️ Zeitbereich zu klein für Slider ({time_diff.total_seconds():.3f}s). Verwende gesamten Bereich."
                        )
                        df_filtered = df
                        return df_filtered
                    else:
                        ...
                        if (
                            ...
                            time_range_seconds = (
                                min_seconds + start_relative,
                                min_seconds + end_relative,
                            )
                            ...
                            st.write(
                                f"**Zeitbereich:** {time_range[0].strftime('%H:%M:%S')} - {time_range[1].strftime('%H:%M:%S')}"
                            )
        ...
        if not single_session_mode:
            ...
            selected_session = st.selectbox(
                ...
                index=(
                    0
                    if st.session_state.get("selected_session", "Alle") == "Alle"
                    else (
                        sessions.index(st.session_state.get("selected_session", "Alle"))
                        if st.session_state.get("selected_session", "Alle") in sessions
                        else 0
                    )
                ),
            )
            ...
            index=(
                ...
                if st.session_state.selected_module == "Alle"
                    ...
                    module_options.index(
                        f"{get_module_icon(st.session_state.selected_module)} {st.session_state.selected_module}"
                    )
