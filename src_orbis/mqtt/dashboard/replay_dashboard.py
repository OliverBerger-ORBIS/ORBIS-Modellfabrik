...
class ReplayDashboard:
    ...
    def _show_replay_controls(self):
        ...
        with col1:
            # Speed control
            speed_options = [0.5, 1.0, 2.0, 5.0, 10.0, 999.0]  # 999.0 = as fast as possible
            speed_labels = ["0.5x", "1x", "2x", "5x", "10x", "Fast as possible"]
            
            speed_index = speed_options.index(st.session_state.replay_speed) if st.session_state.replay_speed in speed_options else 1
        ...
        if workpiece_type:
            ...
            for step in updated_steps:
                ...
                if step['status'] == 'planned':
                    ...
                    st.markdown(f"<div style='opacity: 0.6;'>{step_text} | **{status_text}**</div>", unsafe_allow_html=True)
                else:
    ...
    def _show_production_workflow_analysis(self, filtered_df):
        ...
        if detected_steps:
            ...
            st.info("Keine Fertigungsschritte erkannt. Navigiere durch die Nachrichten, um Order-Informationen zu finden.")
    ...
    def _get_production_steps_for_type(self, workpiece_type):
        ...
        if workpiece_type == "BLUE":
            return [
                {"step": 1, "module": "HBW", "action": "HBW: PICK(DRILL)", "icon": "🏗️", "status": "planned"},
                {"step": 2, "module": "HBW", "action": "HBW: DROP(DRILL)", "icon": "🏗️", "status": "planned"},
                {"step": 3, "module": "FTS", "action": "FTS → Bohrer", "icon": "🚚", "status": "planned"},
                {"step": 4, "module": "DRILL", "action": "Bohrer: PICK(DRILL)", "icon": "🔧", "status": "planned"},
                {"step": 5, "module": "DRILL", "action": "Bohrer: DRILL(DRILL)", "icon": "🔧", "status": "planned"},
                {"step": 6, "module": "DRILL", "action": "Bohrer: DROP(DRILL)", "icon": "🔧", "status": "planned"},
                {"step": 7, "module": "FTS", "action": "FTS → Hochregallager", "icon": "🚚", "status": "planned"},
                {"step": 8, "module": "HBW", "action": "HBW: PICK(MILL)", "icon": "🏗️", "status": "planned"},
                {"step": 9, "module": "HBW", "action": "HBW: DROP(MILL)", "icon": "🏗️", "status": "planned"},
                {"step": 10, "module": "FTS", "action": "FTS → Fräse", "icon": "🚚", "status": "planned"},
                {"step": 11, "module": "MILL", "action": "Fräse: PICK(MILL)", "icon": "⚙️", "status": "planned"},
                {"step": 12, "module": "MILL", "action": "Fräse: MILL(MILL)", "icon": "⚙️", "status": "planned"},
                {"step": 13, "module": "MILL", "action": "Fräse: DROP(MILL)", "icon": "⚙️", "status": "planned"},
                {"step": 14, "module": "FTS", "action": "FTS → KI-Qualitätssicherung", "icon": "🚚", "status": "planned"},
                {"step": 15, "module": "AIQS", "action": "AIQS: PICK", "icon": "🔍", "status": "planned"},
                {"step": 16, "module": "AIQS", "action": "KI-Qualitätssicherung: Qualitätsprüfung", "icon": "🔍", "status": "planned"},
                {"step": 17, "module": "AIQS", "action": "AIQS: DROP", "icon": "🔍", "status": "planned"},
                {"step": 18, "module": "FTS", "action": "FTS → Warenein- und -ausgang", "icon": "🚚", "status": "planned"},
                {"step": 19, "module": "DPS", "action": "Warenein- und -ausgang: DROP", "icon": "📦", "status": "planned"}
            ]
        elif workpiece_type == "RED":
            return [
                {"step": 1, "module": "HBW", "action": "HBW: PICK(MILL)", "icon": "🏗️", "status": "planned"},
                {"step": 2, "module": "HBW", "action": "HBW: DROP(MILL)", "icon": "🏗️", "status": "planned"},
                {"step": 3, "module": "FTS", "action": "FTS → Fräse", "icon": "🚚", "status": "planned"},
                {"step": 4, "module": "MILL", "action": "Fräse: PICK(MILL)", "icon": "⚙️", "status": "planned"},
                {"step": 5, "module": "MILL", "action": "Fräse: MILL(MILL)", "icon": "⚙️", "status": "planned"},
                {"step": 6, "module": "MILL", "action": "Fräse: DROP(MILL)", "icon": "⚙️", "status": "planned"},
                {"step": 7, "module": "FTS", "action": "FTS → KI-Qualitätssicherung", "icon": "🚚", "status": "planned"},
                {"step": 8, "module": "AIQS", "action": "AIQS: PICK", "icon": "🔍", "status": "planned"},
                {"step": 9, "module": "AIQS", "action": "KI-Qualitätssicherung: Qualitätsprüfung", "icon": "🔍", "status": "planned"},
                {"step": 10, "module": "AIQS", "action": "AIQS: DROP", "icon": "🔍", "status": "planned"},
                {"step": 11, "module": "FTS", "action": "FTS → Warenein- und -ausgang", "icon": "🚚", "status": "planned"},
                {"step": 12, "module": "DPS", "action": "Warenein- und -ausgang: DROP", "icon": "📦", "status": "planned"}
            ]
    ...
    def _update_production_steps_from_messages(self, steps, messages_df, current_time=None):
        ...
        for _, message in messages_df.iterrows():
            ...
            if 'SVR3QA2098' in topic:
                if '/pick' in topic.lower() or ('action' in payload_data and 'pick' in str(payload_data.get('action', '')).lower()):
                    completed_steps.add(1)  # HBW PICK (all workflows)
                    completed_steps.add(8)  # HBW PICK (BLUE - second cycle)
                elif '/drop' in topic.lower() or ('action' in payload_data and 'drop' in str(payload_data.get('action', '')).lower()):
            ...
            # MILL PICK, MILL, DROP (SVR4H76449) - for RED and BLUE workflows
                ...
                elif '/mill' in topic.lower() or ('action' in payload_data and 'mill' in str(payload_data.get('action', '')).lower()):
            ...
            # DRILL PICK, DRILL, DROP (SVR4H76530) - for WHITE and BLUE workflows
                ...
                elif '/drill' in topic.lower() or ('action' in payload_data and 'drill' in str(payload_data.get('action', '')).lower()):
            ...
            # DPS DROP (SVR4H73275) - for RED, WHITE, and BLUE workflows
                ...
                if '/drop' in topic.lower() or ('action' in payload_data and 'drop' in str(payload_data.get('action', '')).lower()):
