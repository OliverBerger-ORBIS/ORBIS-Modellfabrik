...
def show_logs():
    ...
    if "session_manager_log_buffer" not in st.session_state:
        ...
        for logger_name in ["session_manager.main", "session_manager.order_analyzer", 
                           "session_manager.auftrag_rot_analyzer", "session_manager.ui_components"]:
    ...
    # Logs anzeigen
    for i, log_entry in enumerate(logs[:100]):  # Max 100 Einträge
        try:
            # Versuche JSON zu parsen
            log_data = json.loads(log_entry)
