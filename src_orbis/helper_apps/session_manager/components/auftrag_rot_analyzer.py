...
class AuftragRotAnalyzer:
    ...
    def find_ccu_order_request(self) -> Optional[Dict[str, Any]]:
        ...
        for message in self.messages:
            if (message.get('topic') == 'ccu/order/request' and 
                message.get('payload')):
                try:
    ...
    def find_related_messages(self, session_data: List[Dict[str, Any]], root_message: Dict[str, Any], time_filter_seconds: Optional[Tuple[float, float]] = None) -> List[Dict[str, Any]]:
        ...
        logger.info(f"🔗 {len(related_messages)} verwandte Messages gefunden (Zeitfenster: {start_time.strftime('%H:%M:%S')} - {end_time.strftime('%H:%M:%S')})")
    ...
    def build_message_chain_graph(self, root_message: Dict[str, Any], related_messages: List[Dict[str, Any]]) -> nx.DiGraph:
    ...
    def create_plotly_graph(self, graph: nx.DiGraph) -> go.Figure:
        ...
        for node in graph.nodes():
            ...
            node_text.append(f"{node}<br>Type: {node_data.get('type', 'unknown')}<br>Topic: {node_data.get('topic', 'N/A')}")
        ...
        fig.update_layout(
            title=dict(
                text='Auftrag-Rot Message Chain Graph',
                font=dict(size=16)
            ),
            showlegend=False,
            hovermode='closest',
    ...
    def analyze_auftrag_rot(self, log_file_path: str, time_filter_seconds: Optional[Tuple[float, float]] = None) -> Dict[str, Any]:
    ...
    if os.path.exists(sessions_dir):
        ...
        if len(topic_messages) == 1:
            ...
            for i, msg in enumerate(topic_messages):
                ...
                payload_preview = str(msg.get('payload', ''))[:50] + "..." if len(str(msg.get('payload', ''))) > 50 else str(msg.get('payload', ''))
