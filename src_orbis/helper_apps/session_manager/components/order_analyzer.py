...
logger = logging.getLogger("session_manager.order_analyzer")
                ...
                if time_range[1] > 0:
                    ...
                    if timestamps:
                        ...
                        filtered_messages = [
                            ...
                            if start_time <= datetime.fromisoformat(msg.get('timestamp', '').replace('Z', '+00:00')) <= end_time
                        ]
    ...
    if st.button("🔍 Order-Chain analysieren", type="primary"):
        with st.spinner("Analysiere Order-Chain..."):
            try:
                ...
                result = _analyze_order_chain(
                    os.path.join(sessions_dir, selected_session),
                    order_id,
                    time_range
                )
