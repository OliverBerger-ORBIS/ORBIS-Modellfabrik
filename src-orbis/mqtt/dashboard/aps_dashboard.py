...
class APSDashboard:
    ...
    def setup_mqtt_client(self):
        ...
        # Only setup MQTT when actually needed
        if not hasattr(self, '_mqtt_setup_done'):
            self._mqtt_setup_done = False
        
        if self._mqtt_setup_done:
            return True
            
        try:
            # Use the latest MQTT client API to avoid deprecation warnings
    ...
    def connect_mqtt(self):
        ...
        try:
            ...
            # Use non-blocking loop instead of loop_start() to avoid threading issues
            self.mqtt_client.loop(0.1)  # Non-blocking loop for 0.1 seconds
    ...
    def disconnect_mqtt(self):
        ...
        if self.mqtt_client:
            try:
                # No loop_stop() needed since we're not using loop_start()
    ...
    def _on_mqtt_connect(self, client, userdata, flags, rc, properties=None):
        ...
        if rc == 0:
            ...
            st.success("✅ MQTT-Verbindung erfolgreich!")
        else:
            self.mqtt_connected = False
            st.error(f"❌ MQTT-Verbindung fehlgeschlagen: {rc}")
    
    def _on_mqtt_disconnect(self, client, userdata, rc, properties=None):
        """MQTT disconnection callback (VERSION2 API)"""
        self.mqtt_connected = False
        st.info("🔌 MQTT-Verbindung getrennt")
    ...
    def send_mqtt_message_direct(self, topic, message):
        ...
        if not self.mqtt_connected:
            if not self.connect_mqtt():
                return False, "MQTT-Verbindung fehlgeschlagen"
    ...
    def load_data(self):
        ...
        try:
            ...
            df = self.extract_module_info(df)
            ...
            if not self.verbose_mode:
                ...
                if original_count != filtered_count:
                    st.sidebar.info(f"📷 {original_count - filtered_count:,} Kamera-Nachrichten ausgeblendet (Verbose-Modus deaktiviert)")
    ...
    def create_filters(self, df):
        ...
        if not df.empty:
            ...
            if not df_filtered.empty:
                # Convert timestamps to time strings for slider
                min_time_str = df_filtered['timestamp'].min().strftime('%H:%M:%S')
                max_time_str = df_filtered['timestamp'].max().strftime('%H:%M:%S')
                
                # Create time options for slider
                time_options = []
                current_time = df_filtered['timestamp'].min()
                end_time = df_filtered['timestamp'].max()
                
                while current_time <= end_time:
                    time_options.append(current_time.strftime('%H:%M:%S'))
                ...
                if len(time_options) > 1:
                    ...
                    df_filtered = df_filtered[
                        (df_filtered['timestamp'].dt.time >= start_time) &
    ...
    def show_message_table(self, df):
        ...
        if not df.empty:
            ...
            display_columns = ['timestamp', 'topic', 'module_type', 'serial_number', 'status', 'process_label', 'session_label']
    ...
    def show_topic_analysis(self, df):
        ...
        if not df.empty:
            ...
            with col2:
                ...
                st.text_area("Alle Topics (erste 1000 Zeichen):", topic_text[:1000], height=200)
    ...
    def show_status_analysis(self, df):
        ...
        if not df.empty:
            ...
            with col4:
                ...
                st.dataframe(payload_type_counts.reset_index().rename(columns={'index': 'Typ', 'payload_type': 'Anzahl'}), use_container_width=True)
    ...
    def show_template_control(self):
        ...
        if selected_template:
            ...
            with col1:
                ...
                st.markdown(f"• **Erwartete Antwort:** {template_info['expected_response']}")
    ...
    def show_custom_order_control(self):
        ...
        with col1:
            # Module selection
            selected_module = st.selectbox("Modul auswählen:", list(self.aps_modules.keys()))
            
            if selected_module:
                # Show module info
                module_info = self.message_library.get_module_info(selected_module)
                working_commands = self.message_library.get_working_commands(selected_module)
        ...
        with col2:
            # Command selection
            if selected_module:
                available_commands = self.message_library.get_working_commands(selected_module)
                selected_command = st.selectbox("Befehl auswählen:", available_commands)
                
                # Metadata options
                st.markdown("**Metadaten:**")
                workpiece_type = st.selectbox("Workpiece Type:", ["WHITE", "BLUE", "RED"])
        ...
        if selected_module and selected_command:
            try:
                ...
                message = self.message_library.create_order_message(selected_module, selected_command, metadata)
                
                st.markdown("**Erstellte MQTT-Nachricht:**")
                st.json(message)
                
                # Send button
                if st.button(f"📤 {selected_module} {selected_command} senden", type="primary"):
                    self.send_custom_mqtt_message(selected_module, selected_command, metadata)
            except Exception as e:
    ...
    def show_aps_analysis(self, df):
        ...
        st.markdown("Interaktive Analyse der persistierten MQTT-Daten")
        st.header("📊 Analyse APS")
        st.markdown("Umfassende Analyse der APS-Daten aus den Sessions")
        
        # Database selection section (moved from sidebar)
        st.subheader("🗄️ Datenbank-Auswahl")
        ...
        if not db_files:
            ...
            st.info("Führe zuerst den Logger aus: `python src-orbis/mqtt/loggers/aps_session_logger.py`")
            return
        ...
        with col1:
            ...
            selected_db = st.selectbox(
                "Datenbank auswählen:",
                db_files,
                index=0 if st.session_state.selected_db is None or st.session_state.selected_db not in db_files else db_files.index(st.session_state.selected_db),
                format_func=lambda x: os.path.basename(x).replace('aps_persistent_traffic_', '').replace('.db', '')
            )
        ...
        with col2:
            # Database info
            if selected_db:
                db_size = os.path.getsize(selected_db) / (1024 * 1024)  # MB
                st.metric("Datenbank-Größe", f"{db_size:.2f} MB")
        
        st.markdown("---")
        
        # Dashboard control section
        st.subheader("🔧 Dashboard-Steuerung")
        ...
        # Verbose mode toggle
        verbose_mode = st.checkbox("🔍 Verbose-Modus (alle Topics anzeigen)", value=self.verbose_mode)
        ...
        analysis_tab1, analysis_tab2, analysis_tab3, analysis_tab4, analysis_tab5, analysis_tab6 = st.tabs([
            "📊 Übersicht", "⏰ Timeline", "📋 Nachrichten", "📡 Topics", "📦 Payload", "🏷️ Sessions"
        ])
    ...
    def show_module_overview_dashboard(self, df):
        ...
        with col3:
            total_commands = sum(len(module['commands']) for module in self.aps_modules_extended.values())
        ...
        for module_key, module_info in self.aps_modules_extended.items():
            ...
            # Connection status
            connection_status = "🟢 Connected" if self.mqtt_connected else "🔴 Disconnected"
                ...
                if i + j < len(module_table_data):
                    ...
                    with cols[j]:
                        ...
                        for command in module_info['commands']:
                            if st.button(f"▶️ {command}", key=f"table_cmd_{module_key}_{command}", use_container_width=True):
    ...
    def show_module_row(self, module_key, module_info, df):
        ...
        with st.container():
            ...
            with col4:
                ...
                for command in module_info['commands']:
                    if st.button(f"▶️ {command}", key=f"cmd_{module_key}_{command}", use_container_width=True):
                        self.send_module_command(module_key, command)
    ...
    def extract_availability_status(self, recent_messages, module_id):
        ...
        try:
            ...
            if 'status' in payload_data:
                ...
                if status in ['AVAILABLE', 'BUSY', 'BLOCKED', 'IDLE', 'RUNNING', 'ERROR']:
                    availability_status = status
            
            # Check state field
            elif 'state' in payload_data:
                state = payload_data['state'].upper()
                if state in ['AVAILABLE', 'BUSY', 'BLOCKED', 'IDLE', 'RUNNING', 'ERROR']:
            ...
            # Check availability field
                ...
                if availability in ['AVAILABLE', 'BUSY', 'BLOCKED', 'IDLE', 'RUNNING', 'ERROR']:
                    availability_status = availability
            
            # Check action status
            elif 'action' in payload_data and 'status' in payload_data['action']:
                action_status = payload_data['action']['status'].upper()
                if action_status in ['AVAILABLE', 'BUSY', 'BLOCKED', 'IDLE', 'RUNNING', 'ERROR']:
    ...
    def show_settings(self):
        ...
        with col1:
            st.metric("Aktive Module", len(self.aps_modules_extended))
            st.metric("Verfügbare Befehle", sum(len(module['commands']) for module in self.aps_modules_extended.values()))
        ...
        with col3:
            st.metric("Datenbank-Dateien", len(glob.glob(os.path.join(os.path.dirname(self.db_file), "*.db"))))
    ...
    def show_mqtt_monitor(self):
        ...
        if self.mqtt_messages_sent:
            ...
            for idx, row in sent_df.head(5).iterrows():
                with st.expander(f"📤 {row['timestamp'].strftime('%H:%M:%S')} - {row['topic']}"):
                    st.json(row['message'])
    ...
    def send_custom_mqtt_message(self, module_name, command, metadata=None):
        ...
        try:
            ...
            message = self.message_library.create_order_message(module_name, command, metadata)
    ...
    def run_dashboard(self):
        ...
        # MQTT connection will be established only when needed (in MQTT Control tab)
        ...
        try:
            ...
            if st.sidebar.button("🏭 Module Overview", use_container_width=True, 
                               type="primary" if st.session_state.selected_tab == "🏭 Module Overview" else "secondary"):
                st.session_state.selected_tab = "🏭 Module Overview"
                st.rerun()
            
            if st.sidebar.button("📊 Analyse APS", use_container_width=True,
                               type="primary" if st.session_state.selected_tab == "📊 Analyse APS" else "secondary"):
                st.session_state.selected_tab = "📊 Analyse APS"
                st.rerun()
            
            if st.sidebar.button("🎮 MQTT Control", use_container_width=True,
                               type="primary" if st.session_state.selected_tab == "🎮 MQTT Control" else "secondary"):
                st.session_state.selected_tab = "🎮 MQTT Control"
                st.rerun()
            
            if st.sidebar.button("⚙️ Einstellungen", use_container_width=True,
                               type="primary" if st.session_state.selected_tab == "⚙️ Einstellungen" else "secondary"):
            ...
            if selected_tab == "🏭 Module Overview":
                self.show_module_overview_dashboard(df)
            elif selected_tab == "📊 Analyse APS":
                self.show_aps_analysis(df)
            elif selected_tab == "🎮 MQTT Control":
                self.show_mqtt_control()
            elif selected_tab == "⚙️ Einstellungen":
                self.show_settings()
...
def main():
    ...
    if not db_files:
        st.error("Keine APS-Datenbanken gefunden!")
        st.info("Führe zuerst den Logger aus: `python src-orbis/aps_persistent_logger.py`")
        return
