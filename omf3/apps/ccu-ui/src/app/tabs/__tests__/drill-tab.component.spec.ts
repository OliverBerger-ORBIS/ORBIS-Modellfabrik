...
import { DrillTabComponent } from '../drill-tab.component';
...
describe('DrillTabComponent', () => {
  let component: DrillTabComponent;
  let fixture: ComponentFixture<DrillTabComponent>;
  ...
  beforeEach(async () => {
    ...
    fixture = TestBed.createComponent(DrillTabComponent);
  ...
  it('should initialize streams', () => {
    expect(component.drillState$).toBeDefined();
    expect(component.connection$).toBeDefined();
    expect(component.drillOrder$).toBeDefined();
  });
  ...
  it('should detect replay mode', async () => {
    ...
    const replayFixture = TestBed.createComponent(DrillTabComponent);
  ...
  it('should get connection status', () => {
    expect(component.getConnectionStatus(mockDrillState)).toBe('ONLINE');
    expect(component.getConnectionStatus(null)).toBe('OFFLINE');
  });

  it('should get availability', () => {
    expect(component.getAvailability(mockDrillState)).toBe('READY');
    expect(component.getAvailability(null)).toBe('UNKNOWN');
  });

  it('should get current action', () => {
    const action = component.getCurrentAction(mockDrillState);
    expect(action).toBeDefined();
    expect(action?.command).toBe('DRILL');
  });

  it('should get recent actions', () => {
    const actions = component.getRecentActions(mockDrillState);
    expect(Array.isArray(actions)).toBe(true);
  });

  it('should get drill depth', () => {
    expect(component.getDrillDepth(mockDrillState)).toBe(15);
    expect(component.getDrillDepth(null)).toBeNull();
  });

  it('should get drill speed', () => {
    expect(component.getDrillSpeed(mockDrillState)).toBe(1200);
    expect(component.getDrillSpeed(null)).toBeNull();
  });

  it('should get workpiece ID', () => {
    expect(component.getWorkpieceId(mockDrillState)).toBe('WP-789012');
  ...
  it('should track actions by ID', () => {
    const action = mockDrillState.actionState;
