 ...
 * Extracted from FtsTabComponent to improve maintainability and reusability
 */
...
export class FtsRouteService {
  ...
  findRoutePath(start: string, target: string): string[] | null {
    ...
    if (!startRef || !targetRef) {
      console.warn('[FtsRouteService] Cannot resolve node refs:', { start, target, startRef, targetRef });
      return null;
    }
    ...
    if (!visited.has(targetRef)) {
      console.warn('[FtsRouteService] No path found:', {
        start,
        target,
        startRef,
        targetRef,
    ...
    console.log('[FtsRouteService] Route path found:', { start, target, startRef, targetRef, path });
    return path;
  }
  ...
  computeStationaryPosition(nodeId: string): ShopfloorPoint | null {
    ...
    if (!moduleCenter) {
      console.warn('[FtsRouteService] Cannot find module center for stationary position:', nodeId, 'resolved:', resolvedId);
      return null;
    }
    ...
    if (!connectedRoad) {
      ...
      if (allConnectedRoads.length === 0) {
        ...
        console.warn('[FtsRouteService] No connected road found for module:', nodeId, 'resolved:', resolvedId);
        return moduleCenter;
      }
