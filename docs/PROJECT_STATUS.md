...
ggf Darstellung in zwei Spalten links die Business-Prozesse, rechts die Shopfloor-Prozesse. Process-Tab wird dann an POsition 3 in der Navileiste verschoben
9. **Orders-Tab Klarstellung** – Tab kommt an 4. Position, Inhaltlich ist die Info ok. GGF Anpassen des generellen Aufbaus an andere Tabs. Testen ob Shopfloor rechtsdargestellt werden sollte und die Iste links. Klarstellen, dass es sich um Shopfloor-Aufträge handelt.  Finished-Liste per Default eingeklappt, letzter Auftrag automatisch expandiert.
