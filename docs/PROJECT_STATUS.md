...
9. **Orders-Tab Klarstellung** – Tab kommt an 4. Position, Inhaltlich ist die Info ok. GGF Anpassen des generellen Aufbaus an andere Tabs. Testen ob Shopfloor links dargestellt werden sollte und die liste der steps rechts. Klarstellen, dass es sich um Shopfloor-Aufträge handelt.  Finished-Liste per Default eingeklappt, letzter Auftrag automatisch expandiert, wenn man die Finished LIste expandiert. Ansonsten Anpassen an die Angular-resize-Strategie, die wir in anderen Tabs verwenden.
