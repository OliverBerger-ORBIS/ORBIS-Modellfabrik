...
**Serial:** Verwendet wird die aktuell ausgewählte AGV-Serial (`selectedAgvSerial$`) – Labels **AGV-1 / AGV-2** entsprechen der **`fts[]`-Reihenfolge** in `shopfloor_layout.json` (aktuell: AGV-1 = **5iO4**, AGV-2 = **xkI4**). Kein hart codiertes Serial in den Command-Payloads.
