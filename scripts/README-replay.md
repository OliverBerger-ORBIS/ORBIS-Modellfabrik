...
Session logs are stored in `data/osf-data/sessions/*.log` and use JSON Lines format (one JSON object per line):
...
```bash
npx tsx scripts/replay-sessions.ts \
  --session data/osf-data/sessions/auftrag-weiss_1.log \
  --once
```
