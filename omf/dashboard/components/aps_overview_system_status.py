...
class APSSystemStatusManager:
    ...
    def update_from_mqtt_client(self, mqtt_client):
        ...
        try:
            ...
            self.total_messages = len(state_messages) + len(connection_messages) + len(pairing_messages) + len(factsheet_messages)
            self.controllers = len(set([msg.get('topic', '').split('/')[3] for msg in state_messages + connection_messages if '/ff/' in msg.get('topic', '')]))
