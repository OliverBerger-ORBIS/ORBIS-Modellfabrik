...
def show_overview_order():
    ...
    with col1:
        ...
        if red_available:
            if st.button(
                "📋 Bestellen", key="overview_customer_order_red", type="secondary", help="Bestellung für ROT Werkstück"
            ):
                _send_order_directly("RED")
        else:
            st.button("📋 Bestellen", key="overview_customer_order_red_disabled", disabled=True)
    ...
    with col2:
        ...
        if blue_available:
            if st.button(
                "📋 Bestellen", key="overview_customer_order_blue", type="secondary", help="Bestellung für BLUE Werkstück"
            ):
                _send_order_directly("BLUE")
        else:
            st.button("📋 Bestellen", key="overview_customer_order_blue_disabled", disabled=True)
    ...
    with col3:
        ...
        if white_available:
            if st.button(
                "📋 Bestellen",
                key="overview_customer_order_white",
                type="secondary",
                help="Bestellung für WHITE Werkstück",
            ):
                _send_order_directly("WHITE")
        else:
            st.button("📋 Bestellen", key="overview_customer_order_white_disabled", disabled=True)
