...
def test_mqtt_client_no_business_logic():
    ...
    assert not hasattr(
        client, "_notify_business_functions"
    ), "❌ MQTT-Client sollte KEINE _notify_business_functions Methode haben"
    assert not hasattr(
        client, "_call_business_function_callback"
    ), "❌ MQTT-Client sollte KEINE _call_business_function_callback Methode haben"

    # Test: Gateway-Referenz existiert
    assert hasattr(client, "_gateway"), "❌ MQTT-Client sollte _gateway Attribut haben"

    # Test: set_gateway Methode existiert
    assert hasattr(client, "set_gateway"), "❌ MQTT-Client sollte set_gateway Methode haben"
...
def test_gateway_routing_logic():
    ...
    assert hasattr(gateway, "on_mqtt_message"), "❌ Gateway sollte on_mqtt_message Methode haben"
