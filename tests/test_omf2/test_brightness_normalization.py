...
class TestBrightnessNormalization:
    ...
    def test_lux_to_percent_basic_ranges(self):
        ...
        for lux, min_percent, max_percent, description in test_cases:
            ...
            assert min_percent <= percent <= max_percent, f"{description}: {lux} lux -> {percent}% (expected {min_percent}-{max_percent}%)"
    ...
    def test_lux_to_percent_mathematical_correctness(self):
        ...
        assert abs(expected_percent - actual_percent) < 0.001, f"Mathematical formula mismatch: expected {expected_percent}, got {actual_percent}"
    ...
    def test_brightness_percentage_ranges(self):
        ...
        for min_percent, max_percent, description in ranges:
            ...
            assert min_percent <= calculated_percent <= max_percent, f"{description}: {lux} lux -> {calculated_percent}% (expected {min_percent}-{max_percent}%)"
