...
def test_request_refresh_basic(refresh_module, mock_redis):
    ...
    with patch('omf2.backend.refresh._get_redis_client', return_value=mock_redis):
        ...
        # Check that timestamp was written to Redis
        timestamp = mock_redis.get('omf2:ui:refresh:test_group')
...
def test_redis_unavailable():
    ...
    with patch.object(refresh, '_get_redis_client', return_value=None):
        # request_refresh should return False
        result = refresh.request_refresh('test_group')
        assert result is False
        
        # get_last_refresh should return None
        timestamp = refresh.get_last_refresh('test_group')
        assert timestamp is None
        
        # clear_refresh should return False
        result = refresh.clear_refresh('test_group')
        assert result is False
        
        # get_all_refresh_groups should return empty list
