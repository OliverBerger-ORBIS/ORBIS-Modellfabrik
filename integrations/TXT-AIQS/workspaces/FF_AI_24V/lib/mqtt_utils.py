...
def _mqtt_create_client():
    global topic, payload, result_code, mqtt_connection, mqtt_config_file, mqtt_controller_id, temp, mqtt_config
...
def mqtt_publish(topic, payload):
    global result_code, mqtt_connection, mqtt_config_file, mqtt_controller_id, temp, mqtt_config
...
def mqtt_get_id():
    ...
    if not mqtt_controller_id:
        mqtt_controller_id = readDeviceIdFromFile()
    return mqtt_controller_id


def mqtt_connect_result_to_string(result_code):
    global topic, payload, mqtt_connection, mqtt_config_file, mqtt_controller_id, temp, mqtt_config
    _results = [
        ' ',
