...
class TestDefaultSession(unittest.TestCase):
    ...
    def test_default_session_exists(self):
        ...
        try:
            ...
            self.assertTrue(
                default_db.exists(), f"Default DB nicht gefunden: {default_db}"
            )
            self.assertTrue(
                default_log.exists(), f"Default Log nicht gefunden: {default_log}"
            )
            ...
            print(
                f"✅ Default-Session existiert: DB={db_size} bytes, Log={log_size} bytes"
            )
    ...
    def test_default_session_loading(self):
        ...
        try:
            ...
            self.assertTrue(
                result, "Dashboard sollte mit Default-Session verbinden können"
            )
    ...
    def test_session_selection_logic(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.assertEqual(
                    len(db_files), 3, "Sollte 3 Sessions haben (inkl. Default)"
                )
                self.assertEqual(
                    db_files[0], default_session, "Default sollte an erster Stelle sein"
                )
