...
class TestFilterIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Insert test data
        test_data = [
            ('module/v1/ff/SVR4H73275/state', '{"status": "available"}', '2025-08-20 10:00:00', 'session-1', 'wareneingang', 'DPS', 'available'),
            ('module/v1/ff/SVR4H76530/state', '{"status": "busy"}', '2025-08-20 10:01:00', 'session-1', 'qualitaetskontrolle', 'AIQS', 'busy'),
            ('module/v1/ff/SVR3QA2098/state', '{"status": "blocked"}', '2025-08-20 10:02:00', 'session-1', 'bearbeitung', 'MILL', 'blocked'),
            ('module/v1/ff/SVR3QA0022/state', '{"status": "available"}', '2025-08-20 10:03:00', 'session-1', 'lagerung', 'HBW', 'available'),
            ('fts/v1/ff/5iO4/state', '{"status": "charging"}', '2025-08-20 10:04:00', 'session-1', 'transport', 'FTS', 'charging'),
            ('module/v1/ff/SVR4H73275/state', '{"status": "available"}', '2025-08-20 11:00:00', 'session-2', 'wareneingang', 'DPS', 'available'),
            ('module/v1/ff/SVR4H76530/state', '{"status": "busy"}', '2025-08-20 11:01:00', 'session-2', 'qualitaetskontrolle', 'AIQS', 'busy'),
        ]
    ...
    def test_data_loading_and_processing(self):
        ...
        try:
            ...
            self.assertEqual(len(df['session_label'].unique()), 2, "Should have 2 sessions")
            self.assertEqual(len(df['module_type'].unique()), 5, "Should have 5 module types")
        ...
        try:
            ...
            for topic in topics:
                ...
                if not topic.startswith('ccu/'):  # CCU topics might not be mapped
                    self.assertNotEqual(friendly_name, topic, f"Topic {topic} should be mapped to friendly name")
    ...
    def test_module_icons_with_real_modules(self):
        ...
        try:
            ...
            for module in modules:
                ...
                self.assertNotEqual(icon, "❓", f"Module {module} should have a proper icon")
