...
class TestFilterImprovements(unittest.TestCase):
    ...
    def test_module_icons(self):
        ...
        try:
            ...
            for module, expected_icons in test_cases:
                ...
                is_valid = (
                    icon in expected_icons  # Exact match
                    or any(
                        expected_icon in icon for expected_icon in expected_icons
                    )  # Contains expected
                    or icon.endswith(".png")
                    or icon.endswith(".jpeg")  # File path
                    or len(icon) == 1  # Single emoji character
                )

                self.assertTrue(
                    is_valid, f"Module '{module}' should have valid icon, got '{icon}'"
                )
    ...
    def test_filter_function_signature(self):
        ...
        try:
            ...
            self.assertIn(
                "single_session_mode",
                params,
                "Function should have 'single_session_mode' parameter",
            )

            # Check default value
            default_value = sig.parameters["single_session_mode"].default
            self.assertEqual(
                default_value, False, "single_session_mode should default to False"
            )
        ...
        try:
            ...
            for option in module_options[1:]:  # Skip 'Alle'
                self.assertIn(
                    " ",
                    option,
                    "Module option should contain space between icon and name",
                )
                parts = option.split(" ", 1)
                self.assertEqual(
                    len(parts), 2, "Module option should have icon and name"
                )
    ...
    def test_friendly_topic_integration(self):
        ...
        try:
            ...
            self.assertEqual(
                friendly_topics["DPS : state"], "module/v1/ff/SVR4H73275/state"
            )
