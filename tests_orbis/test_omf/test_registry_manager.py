...
from src_orbis.omf.tools.registry_manager import (
    Registry, TopicResolver, TopicManager, MessageTemplateManager,
    UnknownTopicError, TemplateMissingError, ValidationError
)
...
def temp_registry():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        order_template = {
            ...
            "module": "DRILL",
            "match": {
                "required_fields": ["orderId", "actionId", "command"],
                "command_enum": ["PICK", "DRILL", "DROP"]
            }
        }
...
def test_command_enum_validation(temp_registry):
    ...
    errors = template_mgr.validate_payload("module/order", {
        "orderId": "test",
        "actionId": "test",
        "command": "INVALID"
    })
