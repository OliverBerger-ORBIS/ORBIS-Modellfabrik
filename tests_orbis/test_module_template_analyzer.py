...
class TestModuleTemplateAnalyzer(unittest.TestCase):
    ...
    def create_test_database(self):
        ...
        test_messages = [
            # HBW State messages
            ("module/HBW/state/status", '{"status": "IDLE", "timestamp": "2025-08-28T10:00:00Z", "module_id": "HBW"}', "2025-08-28T10:00:00Z", "test_session"),
            ("module/HBW/state/status", '{"status": "BUSY", "timestamp": "2025-08-28T10:01:00Z", "module_id": "HBW"}', "2025-08-28T10:01:00Z", "test_session"),
            ("module/HBW/state/position", '{"position": "HOME", "x": 0, "y": 0, "timestamp": "2025-08-28T10:02:00Z"}', "2025-08-28T10:02:00Z", "test_session"),
            
            # VGR Connection messages
            ("module/VGR/connection/status", '{"connected": true, "timestamp": "2025-08-28T10:03:00Z", "ip": "192.168.1.10"}', "2025-08-28T10:03:00Z", "test_session"),
            ("module/VGR/connection/heartbeat", '{"heartbeat": "2025-08-28T10:04:00Z", "status": "OK"}', "2025-08-28T10:04:00Z", "test_session"),
            
            # DPS Order messages
            ("module/DPS/order/request", '{"order_id": "12345", "action": "STORE", "workpiece_id": "040a8dca341291", "timestamp": "2025-08-28T10:05:00Z"}', "2025-08-28T10:05:00Z", "test_session"),
            ("module/DPS/order/response", '{"order_id": "12345", "status": "ACCEPTED", "timestamp": "2025-08-28T10:06:00Z"}', "2025-08-28T10:06:00Z", "test_session"),
            
            # MILL Factsheet messages
            ("module/MILL/factsheet/info", '{"module_id": "MILL", "type": "PROCESSING", "description": "Milling Station", "version": "1.0"}', "2025-08-28T10:07:00Z", "test_session"),
            ("module/MILL/factsheet/status", '{"status": "OPERATIONAL", "maintenance_due": false, "timestamp": "2025-08-28T10:08:00Z"}', "2025-08-28T10:08:00Z", "test_session"),
            
            # OVEN Error messages
            ("module/OVEN/connection/error", '{"error_code": "E001", "message": "Connection timeout", "timestamp": "2025-08-28T10:09:00Z"}', "2025-08-28T10:09:00Z", "test_session"),
            ("module/OVEN/state/error", '{"error_code": "E002", "severity": "HIGH", "message": "Temperature too high", "timestamp": "2025-08-28T10:10:00Z"}', "2025-08-28T10:10:00Z", "test_session"),
        ]
    ...
    def test_determine_sub_category(self):
        ...
        self.assertEqual(self.analyzer._determine_sub_category("module/HBW/connection/status"), "Connection")
        ...
        self.assertEqual(self.analyzer._determine_sub_category("module/MILL/factsheet/info"), "Factsheet")
        self.assertEqual(self.analyzer._determine_sub_category("module/OVEN/unknown/topic"), "General")
    ...
    def test_get_placeholder_for_field(self):
        ...
        self.assertEqual(self.analyzer._get_placeholder_for_field("timestamp", {"2025-08-28T10:00:00Z"}), "<datetime>")
        self.assertEqual(self.analyzer._get_placeholder_for_field("time", {"2025-08-28T10:00:00Z"}), "<datetime>")
        ...
        # Test NFC fields
        self.assertEqual(self.analyzer._get_placeholder_for_field("nfc_code", {"040a8dca341291"}), "<nfcCode>")
        self.assertEqual(self.analyzer._get_placeholder_for_field("workpiece_id", {"040a8dca341291"}), "<nfcCode>")
        
        # Test module fields
        self.assertEqual(self.analyzer._get_placeholder_for_field("module_id", {"HBW", "VGR"}), "<moduleId>")
        
        # Test status fields (enum)
        self.assertEqual(self.analyzer._get_placeholder_for_field("status", {"IDLE", "BUSY"}), "[BUSY, IDLE]")
        
        # Test position fields
        self.assertEqual(self.analyzer._get_placeholder_for_field("position", {"HOME", "WORK"}), "<position>")
        
        # Test error fields
        self.assertEqual(self.analyzer._get_placeholder_for_field("error_code", {"E001", "E002"}), "<errorCode>")
        
        # Test generic string fields
        self.assertEqual(self.analyzer._get_placeholder_for_field("message", {"test1", "test2", "test3"}), "<message>")
        self.assertEqual(self.analyzer._get_placeholder_for_field("description", {"Station 1", "Station 2"}), "[Station 1, Station 2]")
    ...
    def test_save_results(self):
        ...
        test_results = {
            "module/HBW/state/status": {
                ...
                "examples": [
                    {"status": "IDLE", "timestamp": "2025-08-28T10:00:00Z"}
                ],
                "statistics": {"total_messages": 2, "enum_fields": 1, "variable_fields": 1}
            }
        }
...
class TestModuleTemplateAnalyzerIntegration(unittest.TestCase):
    ...
    def create_test_databases(self):
        ...
        for i in range(3):
            ...
            messages = [
                (f"module/HBW/state/status", '{{"status": "IDLE", "timestamp": "2025-08-28T10:00:00Z", "session": {i}}}', "2025-08-28T10:00:00Z", f"session_{i}"),
                (f"module/VGR/connection/status", '{{"connected": true, "timestamp": "2025-08-28T10:01:00Z", "session": {i}}}', "2025-08-28T10:01:00Z", f"session_{i}"),
                (f"module/DPS/order/request", '{{"order_id": "12345", "action": "STORE", "timestamp": "2025-08-28T10:02:00Z", "session": {i}}}', "2025-08-28T10:02:00Z", f"session_{i}"),
            ]
