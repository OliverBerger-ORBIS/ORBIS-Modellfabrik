...
class MessageManager:
    ...
    def validate_message(self, topic: str, message: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        try:
            ...
            logger.info(f"✅ [{self.domain}] Message validation for {topic}: {len(result['errors'])} errors, {len(result['warnings'])} warnings")
            return result
    ...
    def get_latest_message_by_topic(self) -> Dict[str, Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            logger.debug(f"📊 [{self.domain}] Latest messages: {len([m for m in latest.values() if m])} topics with messages")
            return latest
