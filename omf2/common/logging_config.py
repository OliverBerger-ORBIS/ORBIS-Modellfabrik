...
def load_logging_config() -> Dict[str, Any]:
    ...
    if not config_path.exists():
        ...
        return {
            ...
            "business_managers": {"sensor_manager": {"level": "INFO"}, "module_manager": {"level": "INFO"}},
            "mqtt_clients": {"admin_mqtt_client": {"level": "INFO"}, "ccu_mqtt_client": {"level": "INFO"}},
        }
...
def get_current_log_levels() -> Dict[str, str]:
    ...
    architecture_loggers = {
        ...
        "mqtt_clients": ["omf2.ccu.ccu_mqtt_client", "omf2.admin.admin_mqtt_client"],
        # 🚪 Gateways (Architecture Layer 2)
        "gateways": ["omf2.ccu.ccu_gateway", "omf2.admin.admin_gateway"],
        # 🏢 Business Managers (Architecture Layer 3)
        "business_managers": ["omf2.ccu.sensor_manager", "omf2.ccu.module_manager"],
    }

    # Process all architecture loggers
    for _category, logger_list in architecture_loggers.items():
