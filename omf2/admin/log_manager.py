...
class LogEntry:
    ...
    def __init__(
        self, timestamp: datetime, level: LogLevel, component: str, message: str, context: Optional[Dict] = None
    ):
...
class LogManager:
    ...
    def add_log_entry(self, level: LogLevel, component: str, message: str, context: Optional[Dict] = None):
        """Add a log entry"""
        entry = LogEntry(timestamp=datetime.now(), level=level, component=component, message=message, context=context)
    ...
    def clear_logs(self, component: Optional[str] = None):
        ...
        if component:
            ...
            self.buffer.entries = deque(
                [e for e in self.buffer.entries if e.component != component], maxlen=self.buffer.max_size
            )
