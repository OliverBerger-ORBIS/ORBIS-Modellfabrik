...
class RegistryManager:
    ...
    def _load_all_registry_data(self):
        ...
        try:
            ...
            logger.info(
                f"📚 Registry v2 loaded: {len(self.topics)} topics, {len(self.schemas)} schemas, {len(self.workpieces)} workpieces"
            )
                    ...
                    if category not in ["metadata", "category"] and isinstance(topics_list, list):
                        for topic_data in topics_list:
                            if isinstance(topic_data, dict) and "topic" in topic_data:
                                ...
                                topic_info = {"topic": topic_name, "category": file_category, "file": topic_file.name}
