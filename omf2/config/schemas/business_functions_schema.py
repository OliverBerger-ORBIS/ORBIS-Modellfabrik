...
class BusinessFunction(BaseModel if PYDANTIC_AVAILABLE else object):
    ...
    enabled: bool = Field(
        ..., 
        description="Whether this business function is enabled"
    )
    ...
    module_path: str = Field(
        ..., 
        description="Python module path where the class is defined"
    )
    class_name: str = Field(
        ..., 
        description="Name of the class implementing this function"
    )
    routed_topics: List[str] = Field(
        default_factory=list,
        description="List of MQTT topics this function handles"
    )
    ...
    metadata: Optional[BusinessFunctionMetadata] = Field(
        None,
        description="Additional metadata for the function"
    )
...
class QoSSettings(BaseModel if PYDANTIC_AVAILABLE else object):
    ...
    critical_topics: List[str] = Field(
        default_factory=list,
        description="Topics requiring higher QoS"
    )
...
class RoutingConfig(BaseModel if PYDANTIC_AVAILABLE else object):
    ...
    enable_wildcard_matching: bool = Field(
        True,
        description="Enable wildcard matching in topics"
    )
...
class ValidationRules(BaseModel if PYDANTIC_AVAILABLE else object):
    ...
    allow_disabled_functions: bool = Field(
        True,
        description="Allow disabled functions in config"
    )
...
class BusinessFunctionsConfig(BaseModel if PYDANTIC_AVAILABLE else object):
    ...
    qos_settings: Optional[QoSSettings] = Field(
        None,
        description="QoS settings for MQTT"
    )
