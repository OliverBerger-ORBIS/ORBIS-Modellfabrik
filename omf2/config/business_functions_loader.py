...
class BusinessFunctionsLoader:
    ...
    def load_raw(self) -> Dict[str, Any]:
        ...
        if not self.config_file.exists():
            raise FileNotFoundError(f"Business functions config not found: {self.config_file}")
    ...
    def load_validated(self) -> BusinessFunctionsConfig:
        ...
        if not is_pydantic_available():
            logger.warning("Pydantic not available - skipping validation. " "Install pydantic for schema validation.")
        ...
        try:
            ...
            logger.info(
                f"Successfully validated business functions config with " f"{len(config.business_functions)} functions"
            )
            return config
    ...
    def get_enabled_functions(self) -> Dict[str, Dict[str, Any]]:
        ...
        enabled = {name: func for name, func in business_functions.items() if func.get("enabled", False)}
    ...
    def validate_importability(self) -> Dict[str, bool]:
        ...
        if not os.environ.get("ENABLE_IMPORT_CHECK"):
            logger.debug("Import check disabled. Set ENABLE_IMPORT_CHECK=1 to enable.")
            return {}
...
def save_business_functions(config: Dict[str, Any], config_dir: Optional[Path] = None) -> None:
    """
