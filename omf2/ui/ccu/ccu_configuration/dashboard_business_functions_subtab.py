...
def _render_quick_edit_tab(config, loader):
    ...
    try:
        ...
        for idx, (func_name, func_config) in enumerate(business_functions.items()):
            # Create unique title to avoid duplicate ID issues
        ...
        logger.error(f"❌ Quick Edit Tab rendering error: {e}")
        st.error(f"❌ Failed to render Quick Edit Tab: {e}")
        st.info("💡 Please check the business functions configuration.")
...
def _render_yaml_view_tab(config, loader):
    ...
    try:
        ...
        edited_yaml = st.text_area(
            "YAML Content", value=yaml_str, height=400, help="Edit the YAML configuration directly", key="yaml_editor"
        )
        ...
        logger.error(f"❌ YAML View Tab rendering error: {e}")
        st.error(f"❌ Failed to render YAML View Tab: {e}")
        st.info("💡 Please check the YAML configuration format.")
...
def _render_info_tab(config):
    ...
    try:
        ...
        logger.error(f"❌ Info Tab rendering error: {e}")
        st.error(f"❌ Failed to render Info Tab: {e}")
        st.info("💡 Please check the configuration data.")
