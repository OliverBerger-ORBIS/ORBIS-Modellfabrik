...
def _render_order_card(order, production_order_manager, i18n, is_completed=False):
    ...
    with st.container():
        ...
        with col1:
            ...
            _render_order_details(order, production_order_manager, i18n, is_completed)
        
        with col2:
            # Rechts: Shopfloor Layout (nur für aktive Orders)
            if not is_completed:
                _render_shopfloor_for_storage_order(order, production_order_manager, i18n)
            else:
                st.info("🏁 Order completed - no active shopfloor")


def _render_order_details(order, production_order_manager, i18n, is_completed=False):
    ...
    complete_storage_plan = production_order_manager.get_complete_storage_plan(order)
...
def _render_shopfloor_for_storage_order(order, production_order_manager, i18n):
    ...
    storage_plan = production_order_manager.get_complete_storage_plan(order)
    ...
    if active_module:
        if active_module == 'FTS':
            st.info(f"🚗 **FTS Navigation aktiv:** {active_intersections[0] if active_intersections else 'Unknown'} → {active_intersections[1] if len(active_intersections) > 1 else 'Unknown'}")
        else:
