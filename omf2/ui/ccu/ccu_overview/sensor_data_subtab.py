...
def render_sensor_data_subtab(ccu_gateway: CcuGateway, registry_manager):
    ...
    try:
        ...
        with col1:
            ...
            if st.button(f"{UISymbols.get_status_icon('refresh')} {refresh_text}", use_container_width=True, key="sensor_refresh"):
                _refresh_sensor_data()
        
        with col2:
            statistics_text = i18n.t('ccu_overview.sensor_data.show_statistics')
            if st.button(f"{UISymbols.get_functional_icon('dashboard')} {statistics_text}", use_container_width=True, key="sensor_statistics"):
                _show_sensor_statistics(ccu_gateway)
        
        with col3:
            clear_text = i18n.t('ccu_overview.sensor_data.clear_history')
            if st.button(f"{UISymbols.get_functional_icon('settings')} {clear_text}", use_container_width=True, key="sensor_clear"):
    ...
    if bme680_data and bme680_data.get("temperature") is not None:
        ...
        with col1:
            ...
            if thermometer_svg_path.exists() and gradient_svg_path.exists():
                ...
                with open(thermometer_svg_path, "r", encoding="utf-8") as svg_file:
                    svg_content = svg_file.read()
                
                with open(gradient_svg_path, "r", encoding="utf-8") as gradient_file:
                    gradient_content = gradient_file.read()
                    
                # Berechne Füllstand (0-100% für -30°C bis 60°C)
                fill_percentage = max(0, min(100, ((temperature + 30) / 90) * 100))
                
                # SVG-Größe anpassen
                svg_content = svg_content.replace('<svg', f'<svg width="150" height="250"')
                gradient_content = gradient_content.replace('<svg', f'<svg width="150" height="250"')
                ...
                st.caption(f"Bereich: -30°C bis 60°C")
...
def _display_base64_image_compact(base64_data: str, timestamp: str, message_count: int, i18n):
    ...
    try:
        ...
        with col1:
            ...
            st.image(
                image_bytes,
                caption=f"📸 Kamera-Bild ({image_format.upper()})",
                width=400
            )
