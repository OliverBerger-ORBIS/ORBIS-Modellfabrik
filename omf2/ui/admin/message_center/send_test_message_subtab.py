    ...
    try:
        ...
        with col1:
            test_topic = st.text_input(
                "Topic:", value="omf2/test/message", key="test_topic", help="MQTT topic to publish to"
            )
        ...
        with col2:
            qos_level = st.selectbox(
                "QoS Level:", [0, 1, 2], index=1, key="qos_level", help="MQTT Quality of Service level"
            )
            retain_flag = st.checkbox(
                "Retain Message", value=False, key="retain_flag", help="Keep message on broker for new subscribers"
            )
            ...
            if st.button("🚀 Send Message", key="send_test_btn", type="primary", use_container_width=True):
                try:
                    ...
                    success = admin_gateway.publish_message(
                        topic=test_topic, message=message_dict, qos=qos_level, retain=retain_flag
                    )
