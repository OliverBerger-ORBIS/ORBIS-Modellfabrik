...
def render_mqtt_subtab():
    ...
    try:
        ...
        with st.expander("🌐 Broker Settings", expanded=True):
            ...
            st.number_input(
                "Broker Port", min_value=1, max_value=65535, value=1883, key="admin_settings_mqtt_broker_port"
            )
        ...
        with st.expander("⚙️ Client Settings", expanded=False):
            ...
            st.number_input(
                "Keep Alive (seconds)", min_value=10, max_value=600, value=60, key="admin_settings_mqtt_keepalive"
            )
