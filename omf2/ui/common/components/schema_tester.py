...
class SchemaTester:
    ...
    def _test_single_topic(self, topic: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"topic": topic, "status": "ERROR", "error": str(e), "schema_file": None}
    ...
    def _render_test_results(self, test_results: Dict[str, Any]):
        ...
        with col4:
            st.metric(
                "⚠️ No Schema", test_results["no_schema_count"], delta=f"{test_results['no_schema_percentage']:.1f}%"
            )
        ...
        for result in filtered_results:
            with st.expander(
                f"{'✅' if result['status'] == 'VALID' else '❌' if result['status'] == 'INVALID' else '⚠️'} {result['topic']}"
            ):
    ...
    def _export_test_results(self, test_results: Dict[str, Any], export_dir: str):
        ...
        try:
            ...
            # Success message
            st.success("✅ Test results exported successfully!")
            st.info(f"📁 File saved: {file_path}")
            st.info(
                f"📊 Summary: {test_results['valid_count']}/{test_results['total_topics']} topics valid ({test_results['valid_percentage']:.1f}%)"
            )
