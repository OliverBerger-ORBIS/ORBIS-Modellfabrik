...
def last_refresh():
    ...
    try:
        ...
        if not group:
            return jsonify({"success": False, "error": "Missing required parameter: group"}), 400

        # Get last refresh timestamp
        timestamp = get_last_refresh(group)

        if timestamp is not None:
            return jsonify({"success": True, "group": group, "timestamp": timestamp}), 200
        else:
            # No timestamp found (group never refreshed or Redis unavailable)
            return jsonify({"success": True, "group": group, "timestamp": None}), 200
...
def health():
    ...
    """
    return jsonify({"success": True, "status": "ok", "service": "omf2-refresh-api"}), 200
