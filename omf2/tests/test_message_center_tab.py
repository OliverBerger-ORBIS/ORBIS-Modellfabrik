...
class TestMessageCenterTab(unittest.TestCase):
    ...
    def test_render_message_center_tab_basic(self, mock_success, mock_info, mock_columns, mock_markdown, mock_subheader, mock_session_state):
    ...
    @patch('streamlit.info')
    @patch('streamlit.success')
    def test_render_with_deque_buffers(self, mock_success, mock_info, mock_columns, mock_markdown, mock_subheader, mock_session_state):
        ...
        test_buffers = {
            "test/topic1": deque([
                {"data": "test1", "timestamp": time.time() - 100},
                {"data": "test1_new", "timestamp": time.time()}
            ], maxlen=1000),
            "test/topic2": deque([
                {"data": "test2", "timestamp": time.time() - 50}
            ], maxlen=1000)
        }
    ...
    def test_get_latest_timestamp_function(self):
        ...
        test_buffers = {
            "topic1": deque([
                ...
                {"timestamp": 2000, "data": "test1_new"}
            ], maxlen=1000),
            "topic2": deque([
                {"timestamp": 1500, "data": "test2"}
            ], maxlen=1000)
        }
    ...
    def test_deque_buffer_processing(self):
        ...
        buffer = deque([
            {"data": "old", "timestamp": time.time() - 100},
            {"data": "new", "timestamp": time.time()}
        ], maxlen=1000)
