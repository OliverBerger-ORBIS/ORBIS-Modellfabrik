...
class TestStRerunForbidden(unittest.TestCase):
    ...
    def test_no_st_rerun_in_ui_components(self):
        ...
        if violations:
            self.fail(f"🚨 FORBIDDEN st.rerun() USAGE DETECTED in UI components:\n" + 
    ...
    def test_ui_components_import_correctly(self):
        ...
        if import_errors:
            self.fail(f"🚨 UI COMPONENT IMPORT ERRORS:\n" + 
    ...
    def test_no_st_rerun_in_main_dashboard(self):
        ...
        if violations:
            self.fail(f"🚨 FORBIDDEN st.rerun() USAGE DETECTED in main dashboard:\n" + 
                     "\n".join(f"❌ {v}" for v in violations) +
                     "\n💡 SOLUTION: Use request_refresh() from omf2.ui.utils.ui_refresh instead!")
    ...
    def test_only_consume_refresh_in_main(self):
        ...
        if violations:
            self.fail(f"🚨 FORBIDDEN st.rerun() USAGE DETECTED in main application:\n" + 
                     "\n".join(f"❌ {v}" for v in violations) +
    ...
    def test_request_refresh_imported_correctly(self):
        ...
        for file_path in files_to_check:
            if file_path.exists():
                ...
                if "request_refresh" in content and "from omf2.ui.utils.ui_refresh import request_refresh" not in content:
                    violations.append(f"{file_path}: request_refresh() used but not imported correctly")
        
        if violations:
            self.fail(f"🚨 INCORRECT request_refresh() IMPORT:\n" + 
                     "\n".join(f"❌ {v}" for v in violations) +
