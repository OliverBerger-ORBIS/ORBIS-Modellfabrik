        ...
        for payload_file in payload_files:
            try:
                ...
                sequence_result = {
                    'file': payload_file.name,
                    'valid': False,
                    'error': f'Failed to load/validate: {e}'
                }
                result['sequences'].append(sequence_result)
                result['invalid_sequences'] += 1
    ...
    def run_sequence_comprehensive_test(self) -> Dict[str, Any]:
        ...
        for topic in all_topics:
            ...
            if result['status'] == 'ALL_VALID':
                ...
                results['invalid_topics'] += 1
                logger.warning(f"⚠️ {topic}: MIXED ({result['valid_sequences']}/{result['total_sequences']} Sequenzen valid)")
        ...
        # Generate summary
        results['summary'] = {
            'success_rate': (results['valid_topics'] / results['tested_topics'] * 100) if results['tested_topics'] > 0 else 0,
            'coverage': (results['tested_topics'] / results['total_topics'] * 100) if results['total_topics'] > 0 else 0,
            'sequence_success_rate': (results['valid_sequences'] / results['total_sequences'] * 100) if results['total_sequences'] > 0 else 0
        }
        
        logger.info(f"🎯 Test abgeschlossen:")
    ...
    for result in results['topic_results']:
        if result['status'] in ['ALL_INVALID', 'MIXED', 'NO_SCHEMA', 'NO_PAYLOAD']:
            ...
            if result['status'] in ['ALL_INVALID', 'MIXED']:
                print(f"   📋 {result['total_sequences']} Sequenzen: {result['valid_sequences']} valid, {result['invalid_sequences']} invalid")
                for seq in result['sequences']:
                    if not seq['valid']:
                        print(f"   ❌ {seq['file']}: {seq['error']}")
            elif result['status'] == 'NO_SCHEMA':
                print(f"   ⚠️ Kein Schema gefunden")
            elif result['status'] == 'NO_PAYLOAD':
                print(f"   📁 Kein Test-Payload gefunden")
