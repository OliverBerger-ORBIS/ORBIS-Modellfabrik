...
class TestSchemaIntegration(unittest.TestCase):
            ...
            with self.subTest(schema_file=schema_file.name):
                try:
                    ...
                    if schema_type == 'object':
                        self.assertIn('properties', schema_data, f"Object Schema {schema_file.name} sollte 'properties' haben")
    ...
    def test_schema_content_structure(self):
        ...
        for schema_name in test_schemas:
            if schema_name in schemas:
                ...
                self.assertIsInstance(schema_data['properties'], dict, 
                                    f"Schema {schema_name} properties sollte ein Dictionary sein")
    ...
    def test_topic_schema_mapping(self):
        ...
        for topic in test_topics:
            with self.subTest(topic=topic):
                ...
                self.assertTrue(
                    schema is not None or description is not None,
                    f"Topic {topic} sollte Schema oder Description haben"
                )
    ...
    def test_schema_statistics(self):
        ...
        schemas = self.registry_manager.get_schemas()
        self.assertEqual(stats['schemas_count'], len(schemas), 
        ...
        topics_with_schema = sum(1 for topic, info in topics.items() 
                                if isinstance(info, dict) and info.get('schema'))
    ...
    def test_schema_file_naming_convention(self):
        ...
        for schema_file in schema_files:
            with self.subTest(schema_file=schema_file.name):
                ...
                self.assertTrue(schema_file.name.endswith('.schema.json'), 
                              f"Schema {schema_file.name} sollte mit .schema.json enden")
                
                # Prüfe dass Name nicht leer ist
                self.assertGreater(len(schema_file.stem), 0, 
                                 f"Schema {schema_file.name} sollte einen Namen haben")
